...
def generate_job_description(title, level, skills):
    if not setup_gemini():
        return {
            ...
            "requirements": "Configure GEMINI_API_KEY."
        }
    
    prompt = f"""
    You are an expert HR Manager. The user is asking you to write a professional Job Description for a {level} {title}.
...
def calculate_match_score(resume_text, job_desc, job_reqs):
    ...
    try:
        ...
        return perc, ", ".join(missing)
    except Exception as e:
        print(f"Error calculating match score: {e}")
        return 0, "Error generating match score"
